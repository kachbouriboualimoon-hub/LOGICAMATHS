// script.js – Moteur de rendu complet avec rubriques "Résumés", "Fiches", "Aide-mémoire" et "Problèmes"
...
// --- Générateurs de sections fiches (inchangés) ---
...
function renderAideMemoire() {
    ...
    let html = `
        <div class="chapitre-card-style">
            ...
            <div class="chapitre-meta"><span>Cliquez sur une question pour afficher la réponse</span></div>
            <div class="fiches-tabs">
...
function genererAideSection(section) {
    ...
    section.items.forEach((item, index) => {
        const id = 'aide-item-' + index;
        html += `
            <div class="aide-item">
                <div class="aide-question" onclick="toggleAideReponse('${id}')">
                    <span>${item.question}</span>
                    <span class="aide-icon">▼</span>
                </div>
                <div id="${id}" class="aide-reponse">
                    ${item.reponse}
                </div>
            </div>
        `;
...
// --- Basculer l'affichage d'une réponse dans l'aide-mémoire (version ultime) ---
window.toggleAideReponse = function(id) {
    ...
    const icon = parent.querySelector('.aide-icon');
    
    // Basculer l'affichage
    if (el.style.display === 'none' || el.style.display === '') {
        el.style.display = 'block';
        if (icon) icon.classList.add('ouvert');
        // Forcer MathJax après un court délai
        setTimeout(() => {
            if (window.MathJax && MathJax.typesetPromise) {
        ...
        if (icon) icon.classList.remove('ouvert');
    }
};
